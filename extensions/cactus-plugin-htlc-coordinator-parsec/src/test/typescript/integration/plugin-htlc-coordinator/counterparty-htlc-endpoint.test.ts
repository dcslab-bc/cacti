...
test(testCase, async (t: Test) => {
  ...
  const htlcCoordinatorParsecApiClient = new HtlcCoordinatorParsecApi(
    configuration,
  );
  ...
  const response = await htlcCoordinatorParsecApiClient.ownHtlcV1(ownHTLCRequest);
  ...
  const response2 = await htlcCoordinatorParsecApiClient.counterpartyHtlcV1(
    counterpartyHTLCRequest,
  );
