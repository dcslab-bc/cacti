...
export interface IOpenCBDCTestLedgerConstructorOptions {
...
export const OpenCBDC_TEST_LEDGER_DEFAULT_OPTIONS = Object.freeze({
  containerImageVersion: "2024-07-17",
  containerImageName: "opencbdc_test_container",
  rpcApiHttpPort: 8545,
  rpcApiWsPort: 8546,
  envVars: ["OPENCBDC_NETWORK=dev"],
});

export const OPENCBDC_TEST_LEDGER_OPTIONS_JOI_SCHEMA: Joi.Schema = Joi.object().keys(
  {
...
export class OpenCBDCTestLedger implements ITestLedger {
  ...
  constructor(public readonly options: IOpenCBDCTestLedgerConstructorOptions = {}) {
    if (!options) {
      throw new TypeError(`OpenCBDCTestLedger#ctor options was falsy.`);
    }
    this.containerImageVersion =
      options.containerImageVersion ||
      OpenCBDC_TEST_LEDGER_DEFAULT_OPTIONS.containerImageVersion;
    this.containerImageName =
      options.containerImageName ||
      OpenCBDC_TEST_LEDGER_DEFAULT_OPTIONS.containerImageName;
    this.rpcApiHttpPort =
      options.rpcApiHttpPort || OpenCBDC_TEST_LEDGER_DEFAULT_OPTIONS.rpcApiHttpPort;
    this.rpcApiWsPort =
      options.rpcApiWsPort || OpenCBDC_TEST_LEDGER_DEFAULT_OPTIONS.rpcApiWsPort;
    this.envVars = options.envVars || OpenCBDC_TEST_LEDGER_DEFAULT_OPTIONS.envVars;
    ...
    const label = "opencbdc-test-ledger";
    const level = options.logLevel || "INFO";
    this.log = LoggerProvider.getOrCreate({ level, label });
  }

  public getContainer(): Container {
    const fnTag = "OpenCBDCTestLedger#getContainer()";
    if (!this.container) {
  ...
  public async sendEthToAccount(
    ...
    const fnTag = `OpenCBDCTestLedger#sendEthToAccount()`;
  ...
  public async getOpenCBDCKeyPair(): Promise<IKeyPair> {
    const publicKey = await this.getFileContents("/opt/opencbdc/keys/key.pub");
    const privateKey = await this.getFileContents("/opt/opencbdc/keys/key");
  ...
  public async waitForHealthCheck(timeoutMs = 360000): Promise<void> {
    const fnTag = "OpenCBDCTestLedger#waitForHealthCheck()";
  ...
  public stop(): Promise<unknown> {
    const fnTag = "OpenCBDCTestLedger#stop()";
    return new Promise((resolve, reject) => {
      if (this.container) {
  ...
  public destroy(): Promise<unknown> {
    const fnTag = "OpenCBDCTestLedger#destroy()";
    if (this.container) {
  ...
  protected async getContainerInfo(): Promise<ContainerInfo> {
    ...
    if (aContainerInfo) {
      ...
      throw new Error(`OpenCBDCTestLedger#getContainerInfo() no image "${image}"`);
    }
  }

  public async getRpcApiPublicPort(): Promise<number> {
    const fnTag = "OpenCBDCTestLedger#getRpcApiPublicPort()";
  ...
  public async getContainerIpAddress(): Promise<string> {
    const fnTag = "OpenCBDCTestLedger#getContainerIpAddress()";
    const aContainerInfo = await this.getContainerInfo();
  ...
  private validateConstructorOptions(): void {
    const validationResult = OPENCBDC_TEST_LEDGER_OPTIONS_JOI_SCHEMA.validate({
    ...
    if (validationResult.error) {
      throw new Error(
        `OpenCBDCTestLedger#ctor ${validationResult.error.annotate()}`,
      );
    }
  }
}
